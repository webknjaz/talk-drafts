...
* code gating, CI/testing
* linting/autoformatting
* orchestrating workflows
* optimizing images
* backporting
* facilitating release processes
* automate debugging/bisecting problems
* linking knowledge base/docs, related issues
* guessing problem types

-----

## ansibullbot (aka `@ansibot`)

* poll-based, meaning slow action every 10 minutes, unresponsive
* stateful, cannot start processing resource change right away
* monolith, hard to follow the source code, entangled
* hits GitHub's rate limits
* tighly coupled with `ansible/ansible`
* operates using "human" user account

-----

## GitHub Apps

* reusable, installable
* more granular privileges
* don't need accounts pretending to be humans
* granular privileges
...
# GitHub Apps: closer look

<!-- .slide: data-state="alt title" -->

>>>>>

## GitHub Apps: creation

* `Settings` ➔ [`Developer settings` ➔ ] `Github Apps` ➔ `New GitHub App`
  in user or org
* invent a name, fill out necessary fields:
  * webhook events URL
  * requested privileges
* create and download a private key
* write/deploy a web-app accepting webhooks, executing logic and hitting
  GitHub API. Implement corresponding auth flows

>>>>>

## GitHub Apps: installation flow

<center>
![](img/gh-app-install.sequence.svg)
<!-- .element: style="border: none;" -->
</center>

Note:
* user chooses an app, installs it
* accepts permissions, selects acc+repos
* web-app gets notified, creates an access token
...
Note:
* once smth happens, GitHub sends an event using HTTP POST to the URL
  we've specified when setting up the app
* our web-app then accepts it, routes based on event id
...
Note:
now, I'll show you some of my proof of concept GitHub Apps
...
Note:
is suiteable to be run on CIs having Python 3.6+

>>>>>

## ansiwatch: enforce Ansible content compliance Ⓐ

* is an installable GitHub App
* listens to `ping`,
  `integration_installation`, `installation_repositories`
  and `pull_request` events 
* synchronizes a repo to tmp directory periodically
...
* runs `ansible-review` agaist diff in a PR and parses output
  * uses `refs/pull/*/merge` remote read-only namespace
  * uses `git-new-workdir` to feed a checked out copy to `ansible-review`
* posts results to GitHub via Checks API
  * context-aware annotations bound to lines in diff
  * console output as bullet points
  * intermediate statuses and processing states

Note:
* will need to use `git workdir`
...
* is an installable GitHub App
* listens to `ping`, `integration_installation`,
  `installation_repositories`, `pull_request`,
  `check_suite` and `check_run` events 
* does not need to sync a repo
* identifies presence of news fragments just by looking at diff
* posts results to GitHub via Checks API
...
# How to write a bot with GitHub Apps

<!-- .slide: data-state="alt title" -->

Note:
it's not a complete guide, just several remarks on the topic
...
Note:
this all should be solved by a decent framework abstracting routines away
...
* figure out whether you really need a copy of repo
* check out prior art
* use high-level frameworks
* follow good coding practices, don't write 2k LOC long modules
...
## GitHub Apps: API notes

* when authenticated as an app installation, comments/commits get `[bot]`
  label next to the app name in the place where it would show normal
  user's handle
* it is still possible to do actions as a user but this requires
  implemenation of the OAuth 2 flow to get their token
* Checks API is only accessible for GitHub Apps
* Deployment API: abstracts away deployment integrations
* webhook events are the primary action triggers

Note:
webhooks and polling may be combined

>>>>>

## GitHub Apps: Future ideas

* backporting bot, just like miss-islington @ CPython
* CI mediator: integrate Checks API reporting from arbitrary CIs
* news fragments checker
* social surveys: partially automate @gundalow
* release automation via Deployments API

Note:
* Again, ideas may very from typical to exceptionally creative
* Invent yours!

>>>>>

## GitHub Apps: frameworks
...
* [`octomachinery`](https://github.com/sanitizers/octomachinery)
<!-- .element: class="fragment" -->

Note:
I'm currently in progress of developing such a framework

>>>>>

## `octomachinery`: fantastic beast

What if we could just do this?
```python
from octomachinery.app import RUNTIME_CONTEXT, run as run_app
from octomachinery.github import events
...
Note:
* it's a goal I'm heading towards
* this is being extracted from chronographer ATM
...
* https://github.com/sanitizers/octomachinery (TBA)
